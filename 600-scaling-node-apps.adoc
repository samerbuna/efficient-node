...
http
  .createServer((req, res) => {
    for (let i = 0; i < 1e7; pass:[i++]); [gray]##// simulate CPU work##
    res.end();
  })
  .listen(8080, () => {
...
[subs="+quotes,+macros"]
----
[gray]##// Right after the fork loop within the isMaster=true block##
...
setTimeout(() => {
  process.exit(1) [gray]##// death by random timeout##
}, Math.random() \* 10000);
----
