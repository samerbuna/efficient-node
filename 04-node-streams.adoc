...
[subs="+quotes,+macros"]
----
const grep = ... _// A stream for the grep output_
const wc = ...   _// A stream for the wc input_
...
_// Which is equivalent to:_
a.pipe(b);
b.pipe(c);
c.pipe(d);

_// Which, in Linux, is equivalent to:_
_// $ a | b | c | d_
----
...
inStream.push(null); _// No more data_
...
const inStream = new Readable({
  *read*(size) {
    _// there is a demand on the data..._
    _// Someone wants to read it._
  }
});
----
