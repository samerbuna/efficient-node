...
function fileSize (fileName, cb) {
  if (typeof fileName !== 'string') {
    return cb(new TypeError('argument should be string')); [gray]##// Syn [gray]##
  }
  fs.stat(fileName, (err, stats) => {
    if (err) { return cb(err); } [gray]##// Asyn [gray]##
    cb(null, stats.size); [gray]##// Asyn [gray]##
  });
}
----
...
class WithTime extends EventEmitter {
  execute(asyncFunc, ...args) {
    ...
    asyncFunc(...args, (err, data) => {
      if (err) {
        return this.emit('error', err); [gray]##// Not Handled##
      }
