...
[subs="+quotes,+macros"]
----
const grep = ... [gray]##// A stream for the grep output##
const wc = ...   [gray]##// A stream for the wc input##
...
[gray]##// Which is equivalent to:##
a.pipe(b);
b.pipe(c);
c.pipe(d);

[gray]##// Which, in Linux, is equivalent to:##
[gray]##// $ a | b | c | d##
----
...
inStream.push(null); [gray]##// No more data##
...
const inStream = new Readable({
  *read*(size) {
    [gray]##// there is a demand on the data...##
    [gray]##// Someone wants to read it.##
  }
});
----
